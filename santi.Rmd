...
misclass = mean(rand.predictions.test.fit != review.post$review_scores_rating)

boost = boosting(review_scores_rating~., data = review.pre, mfinal = 100, coeflearn = "Freund")
boost.pred = predict.boosting(boost, newdata = review.post)
misclass.boost = boost.pred$error

price.guesses = rand.predictions.test
price.mse = mse.rand.final 
review.guesses = rand.predictions.test.fit
review.acc = misclass
team.name = "TMC"
